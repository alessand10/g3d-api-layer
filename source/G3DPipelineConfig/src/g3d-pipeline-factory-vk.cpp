#include "g3d-pipeline-config/g3d-pipeline-factory-vk.h"
...
void G3DRenderPipelineFactoryVk::createShaderModule(void *bytecode, int bytecodeLength, VkShaderModule_T** shaderModule)
{
    ...
    VkDevice vkDevice = dynamic_cast<G3DDeviceVk*>(m_device)->getDevice();
    ...
    IG3DResourceSetSignature_T** supportedSignatures, uint32_t supportedSignatureCount)
{
    ...
    createShaderModule((void*)createInfo->vertexStage.bytecode, createInfo->vertexStage.bytecodeSize, &pipeline->m_vertexShader);
    createShaderModule((void*)createInfo->fragmentStage.bytecode, createInfo->fragmentStage.bytecodeSize, &pipeline->m_fragmentShader);
...
IG3DComputePipeline_T *G3DRenderPipelineFactoryVk::createComputePipeline(G3DComputePipelineInfo *createInfo, IG3DResourceSetSignature_T **supportedSignatures, uint32_t supportedSignatureCount)
{
    G3DComputePipelineVk* pipeline = new G3DComputePipelineVk();
    pipeline->m_device = dynamic_cast<G3DDeviceVk*>(createInfo->device)->getDevice();

    createShaderModule((void*)createInfo->bytecode, createInfo->bytecodeSize, &pipeline->m_computeShader);
    ...
    VkPipelineShaderStageCreateInfo computeStageCreateInfo {
        ...
        .module = pipeline->m_computeShader,
        .pName = "main",
        .pSpecializationInfo = nullptr
    };
    ...
    if (vkCreateComputePipelines(pipeline->m_device, nullptr, 1U, &pipelineCreateInfo, nullptr, &pipeline->m_pipeline) != VK_SUCCESS) {
